---
date: '2022-10-31T11:43:00'
title: Create Viva Connection for multiple landing pages
description: Guide for creating multiple Connections Apps for Homesites
tags: ['Viva', 'Viva Connections', 'Teams', 'SharePoint']
summary: Guide for creating multiple Connections Apps for Homesites
authors: ['default']
---

So it's not that uncommon that some companies are very decentralized and that they have multiple home sites on a tenant
This usually means that each company/country/etc has its sites in that tenant.
So this is a step to step guide for creating multiple apps!

Requirements:

- Microsoft Teams admin (To add Viva Connection)
- SharePoint admin
- Communication Site
- A custom name for each app
- Custom icons in PNG, 192X192, and 32x32 in size
- Developer portal

Recommended:

- A hub site per Company with hub navigation.
- A theme for each company based on their graphical profile

This is not the full connection suite, you won't get access to a custom dashboard as this requires a home site!

1. Visit Teams and press apps

2. Search after the developer portal

![Image](/static/images/assets/MultipleVivaApps/1.png)

3. Press App and then New app on the top bar

4. This is the example configuration:

# Basic Information

```
| Name                                        | Value                                                |
| ------------------------------------------- | ---------------------------------------------------- |
| Short name - 30 characters or less          | Hub                                                  |
| Full name - up to 100 characters (optional) | Hub                                                  |
| App ID                                      | 8178e53b-f373-4605-87fa-15224cfebf3c //Generated     |
| Short description - 80 characters or less   | Your short description                               |
| Long description - 4,000 characters or less | Your long description                                |
| Developer or company name                   | Your name                                            |
| Website (must be a valid HTTPS URL)         | https://go.microsoft.com/fwlink/?linkid=868076       |
| Privacy policy                              | https://privacy.microsoft.com/en-us/privacystatement |
| Terms of use                                | https://go.microsoft.com/fwlink/?linkid=2039674      |
| Application (client) ID                     | https://YourTentant.sharepoint.com                   |
| Terms of use                                | https://go.microsoft.com/fwlink/?linkid=2039674      |
```

![Image](/static/images/assets/MultipleVivaApps/2.png)
![Image](/static/images/assets/MultipleVivaApps/4.png)

# Branding

![Image](/static/images/assets/MultipleVivaApps/5.png)

# App Feature

```
| Name        | Value                                                                                                                           |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------- |
| Name        | Name of the App                                                                                                                 |
| Entity ID   | Unique ID                                                                                                                       |
| Content URL | https://YourTentant.sharepoint.com/_layouts/15/teamslogon.aspx?spfx=true&dest=https://YourTentant.sharepoint.com/sites/yoursite |
| Website URL | https://YourTentant.sharepoint.com/sites/yoursite                                                                               |
```

![Image](/static/images/assets/MultipleVivaApps/6.png)

# Single sign-on

```
| Name           | Value                              |
| -------------- | ---------------------------------- |
| Application ID | https://YourTentant.sharepoint.com |
```

![Image](/static/images/assets/MultipleVivaApps/7.png)

# Domains:

```

| Domain                             | Source         |
| ---------------------------------- | -------------- |
| YourDomain.sharepoint.com          | Tab,Additional |
| \*.login.microsoftonline.com       | Additional     |
| \*.sharepoint.com                  | Additional     |
| \*.sharepoint-df.com               | Additional     |
| spoppe-a.akamaihd.net              | Additional     |
| resourceseng.blob.core.windows.net | Additional     |
| msft.spoppe.com                    | Additional     |
```

![Image](/static/images/assets/MultipleVivaApps/10.png)

# Languages

Your supported language
![Image](/static/images/assets/MultipleVivaApps/9.png)

# Advanced

## App Content

Check Full-screen mode
![Image](/static/images/assets/MultipleVivaApps/11.png)

# Publish

Press publish to store and press publish
